# AGENTS.md — baramdaero-site

바람대로(에어컨 설치·세척·AS) 본진 정적 사이트. **외부 프로젝트와 자산·코드 혼입 금지.**

## 작업 재개
현재 로컬 수정과 미완료 검증은 [docs/handoff-2026-09-05.md](docs/handoff-2026-09-05.md)를 먼저 확인한다. 기존 사이트 마감과 브랜드 영상 작업을 구분하고 실제 HEAD·diff부터 대조한다.
상업용 안내·공간 이미지와 문의 분기 추가는 [docs/handoff-2026-09-06-commercial.md](docs/handoff-2026-09-06-commercial.md)에 이어 기록했다.
...
4. **기준 문서가 원본이다.** 카피·콘텐츠 작업 전에 `docs/standards/`를 먼저 읽는다
   (0-원칙 / 1-작업기준 / 2-발화기준 / 3-데이터기준). 이 AGENTS.md와 충돌하면 기준 문서를 따른다
...
## 이미지·영상 참조
시스템에어컨이 등장하는 장면은 `../baramdaero-reference-assets/INDEX.md`와 `README.md`를 먼저 읽는다. 삼성·LG 공식 제품 정본을 장면별 참조로 사용하는 Google Flow 방식으로 작업하며 기존 생성 자산부터 확인한다. 공식 참고 원본은 공개 레포에 복사하지 않는다. **예외(2026-09-21, 박민혁 승인)**: 홈 시스템에어컨 섹션의 1Way 제품 사진 4장(`public/media/ac-1way-{samsung,lg}-{front,angle}.webp`)은 제조사 공식 제품컷을 흰 배경만 잘라내 게재하고 파일도 이 레포에 둔다. 승인 범위는 이 4장이며, 다른 공식 원본은 계속 복사하지 않는다. **현재 게재는 삼성 2장(front·angle)뿐이다** — 2026-09-22 에 탭을 기종 단위로 바꾸면서 LG 1Way 탭이 없어졌다(부품 구성이 삼성과 같아 브랜드 차이를 주장할 근거가 없다). LG 2장은 승인 범위 안이라 미참조 상태로 남겨 둔다 — 지우지 말 것, 다시 쓸 때 승인을 새로 받게 된다. 현 적용 이미지 이력은 상업용 인계 문서에 있다. 반복 제작 절차는 `flow-product-reference` 스킬을 사용한다.
...
- **카피 중복을 셀 때는 화면 본문과 인라인 데이터를 구분해서 센다.** 챗봇 데이터
  (`faq.json` 등)는 `dist` HTML에 JSON으로 박혀 grep에 잡히지만, 화면에는 패널을
  열 때만 노출된다. 본문 중복과 데이터 중복은 성격이 다르다 — 합쳐 세면
  고칠 필요 없는 것을 고치게 된다 (08-25 `/care/` 중복 카운트에서 최초 관측)
...
## JSON 데이터 파일 편집 절차 (2026-08-25 강제)

`src/content/site/*.json`은 **문자열 치환으로 수정한다.**
`json.dumps` 등으로 파일 전체를 다시 쓰면 배열 압축 표기 같은 기존 포맷이 풀려
diff가 불필요하게 커진다 — 고친 줄이 포맷 변경에 묻혀 리뷰가 불가능해진다.
(2026-08-25 실측: `faq.json` 2줄 고치려다 `related` 배열이 풀려 18줄 삽입)

- 파싱은 읽기·검증용으로만 쓴다. 쓰기는 원문 문자열에 `replace`
- 치환 전 `count == 1`로 대상이 유일한지 확인한다 (같은 문장이 여러 필드에 있을 수 있다)
- 필드를 특정해야 하면 키까지 포함해 치환한다 (`"a": "…"` 형태)
