...
## 이미지·영상 참조
시스템에어컨이 등장하는 장면은 `../baramdaero-reference-assets/INDEX.md`와 `README.md`를 먼저 읽는다. 삼성·LG 공식 제품 정본을 장면별 참조로 사용하는 Google Flow 방식으로 작업하며 기존 생성 자산부터 확인한다. 공식 참고 원본은 공개 레포에 복사하지 않는다. 현 적용 이미지 이력은 상업용 인계 문서에 있다. 반복 제작 절차는 `flow-product-reference` 스킬을 사용한다.
