...
- **모바일 헤더(640px 이하)는 '메뉴' 버튼 + 펼침 목록** (2026-09-24). 메뉴 칸이 6개가 되면서 한 줄(칸마다 44px)에 안 들어가
  버튼으로 접었다. 버튼은 글자('메뉴'↔'닫기') — 아이콘은 koboyo 에서만 가져오는데 연결 전이라서다. 목록은 전체 명칭·한 줄 52px,
  Esc·바깥 누르기로 닫힌다. JS 가 안 돌면 예전 한 줄 메뉴가 남고 '가격' 칸만 숨는다(넘침 방지)
