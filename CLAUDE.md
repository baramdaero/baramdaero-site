...
서비스 범위·가격·기간·약속에 해당하는 정책이 바뀌면 한 곳만 고치고
끝내지 않는다. 아래 채널을 전수 확인한다.
...
- `situations` / `cases` / `blog` 콘텐츠
- 각 페이지 하드코딩 문구

검사는 **dist 전수 grep**으로 한다 (HTML + 번들 JS 모두 — 위 §카피 작업
완료 조건 참조). 소스만 보면 챗봇 COPY가 번들로 들어가 HTML grep에 안 잡힌다.
